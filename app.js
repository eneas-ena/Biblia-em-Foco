...
const state = {
    ...
    const els = {
      ...
      mobileStudyOverlay: document.getElementById("mobileStudyOverlay"),
      mobileStudySurface: document.getElementById("mobileStudySurface"),
    ...
    const mobilePanelOrigins = new Map();
...
Object.assign(appData.lexicon, loadCustomLexicon());
    ...
    // === Mobile panel v11: clean rebuild ===========================================
    // Mobile no longer depends on the desktop aside order. When a word is selected,
    // the Strong card is rendered into a dedicated mobile overlay, above notes,
    // favorites and "Meu estudo". Desktop still uses the original aside.

    function renderMobileStudyCard() {
      if (!els.mobileStudyContent) return;
      restoreMobilePanels();
      const panels = [
        document.querySelector(".notes-panel"),
        document.querySelector(".favorites-panel"),
        document.querySelector(".my-study-panel")
      ];

      els.mobileStudyContent.innerHTML = `
        <button id="mobileOverlayBack" class="mobile-study-back secondary" type="button">
          <span aria-hidden="true">‹</span>
          <strong>Texto</strong>
          <small>deslize para voltar</small>
        </button>
        <section id="mobileStudyCard" class="study-card mobile-strong-card"></section>
      `;

      const mobileTarget = els.mobileStudyContent.querySelector("#mobileStudyCard");
      if (state.markingAssistant) {
        const verse = getChapter().verses.find(item => item.n === state.markingAssistant.verseNumber);
        if (verse) {
          renderMarkingAssistant(state.markingAssistant.word, verse, mobileTarget);
        } else {
          renderStudyCard(mobileTarget);
        }
      } else {
        renderStudyCard(mobileTarget);
      }
      ...
      panels.forEach(panel => {
        if (!panel) return;
        mobilePanelOrigins.set(panel, { parent: panel.parentNode, next: panel.nextSibling });
        els.mobileStudyContent.appendChild(panel);
      });

      els.mobileStudyContent.querySelector("#mobileOverlayBack")?.addEventListener("click", closeMobileStudyPanel);
    }

    function restoreMobilePanels() {
      Array.from(mobilePanelOrigins.entries()).reverse().forEach(([panel, { parent, next }]) => {
        if (!parent) return;
    ...
    function mountMobileStudyScreen() { renderMobileStudyCard(); return true; }
    function unmountMobileStudyScreen() {}

    function setMobilePanelContentVisibility(mode) {
      if (!els.side) return;
      if (!isMobileReader()) { resetMobilePanelContentVisibility(); return; }
    ...
    function openMobileStudyPanel(mode = "study", options = {}) {
      ...
      if (els.mobileStudyOverlay) {
        els.mobileStudyOverlay.hidden = false;
        els.mobileStudyOverlay.setAttribute("aria-hidden", "false");
      }
      if (els.mobileStudySurface) {
        els.mobileStudySurface.dataset.panelMode = mode;
        const resetScroll = () => { els.mobileStudySurface.scrollTop = 0; };
    ...
    function closeMobileStudyPanel() {
      ...
      resetMobilePanelContentVisibility();
      if (els.mobileStudyOverlay) {
        els.mobileStudyOverlay.hidden = true;
        els.mobileStudyOverlay.setAttribute("aria-hidden", "true");
      }
      if (els.mobileStudySurface) {
        delete els.mobileStudySurface.dataset.panelMode;
        els.mobileStudySurface.style.removeProperty("--mobile-panel-shift");
      }
      restoreMobilePanels();
      if (els.mobileStudyContent) {
        els.mobileStudyContent.innerHTML = "";
      }
    }

    function bindMobileStudyGestures() {
      if (!els.mobileStudySurface) return;
      let startX = null;
      let startY = null;
      const target = els.mobileStudySurface;
    ...
    function openMarkingAssistant(el) {
      ...
      state.markingAssistant = { word, verseNumber };
      renderMarkingAssistant(word, verse, els.studyCard);
      openMobileStudyPanel("study", { render: false });
    }

    function renderMarkingAssistant(word, verse, target) {
      target = target || els.studyCard;
      if (!target) return;
      ...
      target.innerHTML = `
        ...
        <div class="assistant-panel">
          ...
          <div class="assistant-actions">
            <button class="secondary" type="button" data-action="suggestStrongAi">Sugerir com IA</button>
            <button class="secondary" type="button" data-action="copyStrongPrompt">Copiar prompt</button>
          </div>
          <div class="ai-response" data-role="strongAiResponse"></div>
        </div>
      `;

      target.querySelectorAll("[data-suggest-strong]").forEach(button => {
        button.addEventListener("click", () => {
          addCustomStrongMark(Number(button.dataset.suggestVerse), button.dataset.suggestWord, button.dataset.suggestStrong);
        });
      });

      const suggestStrongAi = target.querySelector('[data-action="suggestStrongAi"]');
      if (suggestStrongAi) {
        suggestStrongAi.addEventListener("click", () => runStrongSuggestion(word, verse, suggestions, target));
      }

      const copyStrongPrompt = target.querySelector('[data-action="copyStrongPrompt"]');
      if (copyStrongPrompt) {
    ...
    async function runStrongSuggestion(word, verse, currentSuggestions = [], target) {
      const scope = target || document;
      const responseBox = scope.querySelector('[data-role="strongAiResponse"]') || document.querySelector('[data-role="strongAiResponse"]');
      const runButton = scope.querySelector('[data-action="suggestStrongAi"]') || document.querySelector('[data-action="suggestStrongAi"]');
      ...
      try {
        ...
        responseBox.querySelectorAll("[data-ai-strong]").forEach(button => {
          button.addEventListener("click", () => {
    ...
    function renderStudyCard(target = els.studyCard) {
      ...
      if (!state.selected) {
        target.innerHTML = `
          <div class="card-header">
            <span class="label">Marcação assistida</span>
            <h3>Escolha uma palavra marcada</h3>
            <p>Toque em uma palavra destacada no texto para abrir o estudo.</p>
          </div>
          <div class="assistant-panel">
            <p class="prose">As informações de Strong, exegese, paralelos e análise com IA aparecerão aqui quando uma palavra marcada for selecionada.</p>
          </div>
        `;
        return;
      }
