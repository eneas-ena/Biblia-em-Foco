...
Object.assign(appData.lexicon, loadCustomLexicon());
    ...
    // === Mobile drawer v10 (Strong card pinned first) ==============================
    ...
    function pinStudyCardFirstInMobilePanel() {
      if (!els.side || !els.studyCard) return;
      if (els.mobileStudyBack?.parentElement === els.side) {
        const firstPanelAfterBack = els.mobileStudyBack.nextElementSibling;
        if (firstPanelAfterBack !== els.studyCard) {
          els.side.insertBefore(els.studyCard, els.mobileStudyBack.nextSibling);
        }
        return;
      }
      if (els.side.firstElementChild !== els.studyCard) {
        els.side.insertBefore(els.studyCard, els.side.firstChild);
      }
    }
    ...
    function setMobilePanelContentVisibility(mode) {
      ...
      // Keep the drawer content intact on mobile. Earlier versions hid every
      // sibling of the Strong card with inline display:none, which made notes,
      // favorites and "Meu estudo" disappear after tapping a word.
      resetMobilePanelContentVisibility();

      if (studyMode && els.studyCard) {
        pinStudyCardFirstInMobilePanel();
        els.studyCard.hidden = false;
        els.studyCard.style.setProperty("display", "block", "important");
        els.studyCard.style.setProperty("visibility", "visible", "important");
        els.studyCard.style.setProperty("opacity", "1", "important");
        els.studyCard.style.setProperty("order", "-1", "important");
      }
    }
    ...
    function openMobileStudyPanel(mode = "study", options = {}) {
      ...
      if (els.side) {
        ...
        const resetScroll = () => {
          if (mode === "study") setMobilePanelContentVisibility("study");
